...
def _is_bad_sentence(sen1, sen2):
    
    tok1 = sen1.lower().split()
    tok2 = sen2.lower().split()
    if len(sen1) < 5:
        return True
    if ' '.join(tok1[:3]) in sen2:
        return True

    return False


def clean_summary(input_):

    output_ = []
    for sen1 in input_:
        is_bad = False
        for sen2 in output_:
            if _is_bad_sentence(sen1, sen2):
                is_bad = True
                break
        if is_bad:
            continue
