...
def process_minibatch(batch_id, path_,
    ...
    ttl_input_arr = [
        itm[:-1] + [vocab2id['<pad>']
                    for _ in range((1+ttl_max_lens-len(itm)))]
        for itm in ttl_arr
    ]
    ttl_output_arr = [
        itm[1:] + [vocab2id['<pad>'] for _ in range((1+ttl_max_lens-len(itm)))]
        for itm in ttl_arr
    ]
    sum_input_arr = [
        itm[:-1] + [vocab2id['<pad>']
                    for _ in range((1+sum_max_lens-len(itm)))]
        for itm in sum_arr
    ]
    sum_output_arr = [
        itm[1:] + [vocab2id['<pad>'] for _ in range((1+sum_max_lens-len(itm)))]
        for itm in sum_arr
    ]
    src_arr = [
        itm + [vocab2id['<pad>'] for _ in range((src_max_lens-len(itm)))]
        for itm in src_arr
    ]
...
def process_minibatch_explicit_test(batch_id, path_,
    ...
    src_idx_ex = [itm + [vocab2id['<pad>'] for _ in range(src_lens-len(itm))]
                  for itm in src_idx_ex]
    src_var_ex = Variable(torch.LongTensor(src_idx_ex))

    src_wt = [itm[:src_lens] for itm in src_wt]
    src_wt = [itm + [0.0 for _ in range(src_lens-len(itm))] for itm in src_wt]
    src_msk = Variable(torch.FloatTensor(src_wt))
