...
from .end2end_uae import End2EndUAEBase


class modelUAEBase(End2EndUAEBase):
    '''
    ...
    def get_aspect_keywords(self):
        '''
        Get keywords for each aspect
        '''
        ...
        wdidx = score.topk(k=1, dim=0)[1][0]
        vocab2asp = {}
        vocab = {}
        for wd in self.batch_data['vocab2id']:
            idx = self.batch_data['vocab2id'][wd]
            asp = wdidx[idx].data.cpu().numpy().tolist()
            vocab2asp[idx] = asp+1

        top_idx = score.topk(
            k=self.args.n_keywords, dim=1)[1].cpu().numpy().tolist()

        return top_idx, vocab2asp
    ...
    def build_batch(self, batch_id):
        ...
        if batch_id % 20 == 0:
            _, vocab2asp = self.get_aspect_keywords()
            self.batch_data['vocab2asp'] = vocab2asp
        try:
            vocab2asp = self.batch_data['vocab2asp']
        except:
            _, vocab2asp = self.get_aspect_keywords()
            self.batch_data['vocab2asp'] = vocab2asp
        ...
        for line in fp:
            itm = json.loads(line)
            senid = [
                vocab2id[wd] for wd in itm['text_fine']
                if wd in vocab2id]
            if self.args.task == 'train' and len(senid) < 3:
                continue
            sen_text.append(senid)
            sen_tok = [
                wd for wd in itm['text_fine'] if wd in vocab2id]
            ...
            try:
                ...
                labels.append('None')
            asp_ = [vocab2asp[wd] for wd in senid]
            asp_mapping.append(asp_)
        fp.close()
        ...
        asp_mapping = [
            itm[:sen_text_len]
            + [0 for _ in range(sen_text_len-len(itm))]
            for itm in asp_mapping]
        asp_mapping_var = Variable(torch.LongTensor(
            asp_mapping)).to(self.args.device)

        self.batch_data['sen_tokens'] = sen_tokens
        self.batch_data['pos_sen_var'] = sen_text_var
        self.batch_data['pos_pad_mask'] = sen_pad_mask
        self.batch_data['asp_mapping'] = asp_mapping_var
        ...
        if self.args.task == 'train':
            ...
            for k in range(20):
                neg_asp_mapping = neg_asp_mapping[1:] \
                    + [neg_asp_mapping[0]]
                neg_asp_mapping_var = Variable(torch.LongTensor(
                    neg_asp_mapping)).to(self.args.device)
                ...
                neg_examples.append(
                    [neg_text_var, neg_pad_mask, neg_asp_mapping_var])
    ...
    def aspect_worker(self):
        ...
        Aspect keywords
        '''
        top_idx, _ = self.get_aspect_keywords()
