...
def visualization(args):
    ...
    docs = read_docs(os.path.join(
        args.work_dir, args.file_term_doc))
    vocab = read_vocab(os.path.join(
        args.work_dir, args.file_vocab_out))
    n_docs = len(docs)
    n_terms = len(vocab)
    ...
    W = np.loadtxt(os.path.join(
        args.work_dir, args.file_aspect), dtype=float)
    n_topic = W.shape[1]
    ...
    for k in tqdm(range(n_topic)):
        topKeywordsIndex = []
        for wd in W[:, k].argsort()[::-1]:
            topKeywordsIndex.append(wd)
        topKeywordsIndex = topKeywordsIndex[:args.n_keywords]
    ...
    for k in index:
        kw_idx = []
        for wd in np.argsort(W[:, k])[::-1]:
            kw_idx.append(wd)
        kw_idx = kw_idx[:args.n_keywords]
