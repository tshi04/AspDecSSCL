...
class modelUAE(modelNLGBase):
    ...
    def build_models(self):
        ...
        self.train_models['aspect_attn_self'] = AttentionSelf(
            input_size=self.args.emb_size,
            hidden_size=self.args.emb_size,
    ...
    def build_pipe(self):
        ...
        with torch.no_grad():
            ...
            asp_emb = self.base_models['embedding'](
                self.batch_data['aspect_terms'])
        src_enc, hidden_encoder = self.train_models['encoder'](src_emb)

        _, asp_enc = self.train_models['aspect_attn_self'](asp_emb)
        asp_enc = asp_enc.unsqueeze(0).repeat(bsize, 1, 2)

        if self.args.task == "train":
            mean = Variable(torch.zeros(
                asp_enc.size())).to(self.args.device)
            std = Variable(torch.ones(
                asp_enc.size())).to(self.args.device)
            var = torch.normal(mean, 0.1*std)
            asp_enc = asp_enc + var

        attn_aspect = asp_enc @ src_enc.transpose(1, 2)
