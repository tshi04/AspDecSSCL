    ...
    def nmf_iter(self):
        ...
        for i in range(self.max_iter):
            ...
            if loss_old-loss < self.max_err:
                break
            end_time = time.time()
            print('Step={}, Loss={}, Loss_diff={}, Time={}s'.format(
                i, np.around(loss, 4),
                np.around(loss_old-loss, 4), end_time-start_time))
            loss_old = loss
...
class NMF(object):
    ...
    def nmf_solver(self):
        ...
        for k in range(self.n_topic):
            tmpW = self.W[:, k]*HtH[k, k] + \
                AH[:, k] - np.dot(self.W, HtH[:, k])
        ...
        for k in range(self.n_topic):
            self.H[:, k] = self.H[:, k]*WtW[k, k] + \
                AtW[:, k] - np.dot(self.H, WtW[:, k])
            self.H[:, k] = np.maximum(self.H[:, k], epss)
